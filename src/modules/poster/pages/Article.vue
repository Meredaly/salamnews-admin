<script setup lang="ts">
  ...
  const getArticle = async () => {
    try {
      ...
      article.value = {
        ...
        time_of_event: data.time_of_event || null,
        main_photo: {
        ...
        main_video: {
          ...
          cutting_error: !!data.main_video?.cutting_error,
          cutting_status: !!data.main_video?.cutting_status
        }
      }
  ...
  const deleteMainPhoto = (index: number) => {
    article.value.main_photo.photo.splice(index, 1)
  }
  ...
  const changeContentVideo = async (index: number, file: File) => {
    ...
    if (data.status === 'SUCCESS') {
      ...
      article.value[index].duration = data.duration
  ...
  const showTimeOfEventCalendar = ref(false)
  const openTimeOfEventCalendar = () => {
    showTimeOfEventCalendar.value = true
  }
  const closeTimeOfEventCalendar = () => {
    showTimeOfEventCalendar.value = false
  }
  const clearTimeOfEvent = () => {
    article.value.time_of_event = null
  }

  const selectTimeOfEventCalendar = ({ date, time }: { date: string; time: string }) => {
    article.value.time_of_event = `${date} ${time}`
    closeTimeOfEventCalendar()
  }
...
<template>
    ...
    <div class="article__container">
      <div class="article__wrapper">
        ...
        <div class="article__body">
          ...
          <div class="article__author-name">
            <article-textarea v-model.trim="article.price" :placeholder="t('price')" size="LARGE" />
          </div>
          <div class="article__info-pin">
            <div class="article__info">
              <article-info :info="articleInfo" />
            </div>
            <div class="article__time-of-event">
              <div v-if="article.time_of_event" class="article__time-of-event-date">
                <icon name="calendar" />
                <p>{{ t('eventTime') }}: {{ getDateTime(article.time_of_event) }}</p>
              </div>
              <s-button
                v-if="!article.time_of_event"
                @click="openTimeOfEventCalendar"
                :title="t('addEvenTime')"
                prev-icon="calendar"
                styles="light"
              />
              <s-button v-if="article.time_of_event" @click="clearTimeOfEvent" styles="light" prev-icon="closeCircle" />
            </div>
          </div>
          ...
          <div v-if="selectedTab.id === 'PHOTO'" class="article__slider">
            <article-slider-uploader
              v-model:author="article.main_photo.author"
              v-model:description="article.main_photo.description"
              @change-file="changeMainPhoto"
              @delete-image="deleteMainPhoto"
              :photos="article.main_photo.photo"
            />
          </div>
          <div v-if="selectedTab.id === 'VIDEO'" class="article__main-video">
            <article-video-uploader
              @change-poster-file="changeMainPosterFile"
              @change-video-file="changeMainVideoFile"
              v-model:author="article.main_video.author"
              v-model:description="article.main_video.description"
              :poster="article.main_video.poster"
  ...
  <teleport to="#pop-up">
    ...
    <article-plan
      v-if="showTimeOfEventCalendar"
      :date="article.time_of_event"
      @cancel="closeTimeOfEventCalendar"
      @save="selectTimeOfEventCalendar"
    />
    <pop-up-confirm
...
<style lang="scss" scoped>
  .article {
    ...
    // .article__time-of-event
    &__time-of-event {
      display: flex;
      gap: 4px;
    }
    // .article__time-of-event-date
    &__time-of-event-date {
      display: flex;
      align-self: center;
      gap: 6px;
      border-radius: 4px;

<script setup lang="ts">
  ...
  const normalizeTaskDataMainInfo = (task: TaskData): NewsInfoSet[] => {
    ...
    const langsInfo = task.langs
      .sort((x, y) => LANG_ORDER.indexOf(x.lang_name) - LANG_ORDER.indexOf(y.lang_name))
      .map((taskLang: TaskDataLanguage): NewsInfoSet => {
  ...
  const deleteTask = async (dropResult: DropResult) => {
    try {
      ...
      if (dropResult.removedIndex !== null || dropResult.addedIndex !== null) {
        await DELETE_TASK_LANG({ id: dropResult.payload.uuid })
        getTasks()
      }
    } catch (error) {
      console.error(error)
...
<template>
  ...
  <teleport to="#pop-up">
    <pop-up-task v-if="showTask" @close="closeTask" @save="addTask" title="Добавить афишу" />
  </teleport>
</template>

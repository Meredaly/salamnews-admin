<script setup lang="ts">
  ...
  import { GET_PLANNED } from '../../api/whiteboard'
  ...
  const getPlannedData = async (): Promise<void> => {
    try {
      const { data } = await GET_PLANNED({
        params: {
      ...
      planned.value =
        data.get_news_planning?.map((news: any) => {
          news.published_date = news.time_of_event
          return news
        }) || []
    } catch (error) {
      console.error(error)
  ...
  const goToPath = () => {
    router.push({
      path: `/poster/working-environment/planned`,
      query: {
        lang: currentLang.value?.id || 'TKM'
      }
    })
    getPlannedData()
  }
  ...
  const openNews = ({ article, index }: any) => {
    index
    router.push(`/poster/article/${article.uuid}`)
  }
...
<template>
  <div class="wait-list">
    ...
    <div class="wait-list__articles">
      <articles-horizontal :articles="planned" @open="openNews" />
    </div>
  </div>
</template>

<script setup lang="ts">
  ...
  const getWatan = async () => {
    try {
      const { data } = await GET_WATAN({ uuid: route.params.id })
      ...
      articleStatus.value = {
        uuid: route.params.id,
        status: data.status,
        publication_date: data.published_date || data.publication_date || null,
        notification: false
      }
  ...
  const updateVideoReport = async () => {
    try {
      ...
      await UPDATE_WATAN({ data: article.value })
  ...
  const confirmDelete = async () => {
    closeConfirmPopUp()
    const status = await deleteWatan()
    if (status) {
      router.go(-1)
    }
  }

  const deleteWatan = async () => {
    try {
      await DELETE_WATAN({ uuid: article.value.uuid })
      return true
    } catch (error) {
      console.error(error)
      return false
  ...
  const updateStatus = async () => {
    try {
      await UPDATE_WATAN_STATUS({ data: articleStatus.value })
      router.go(-1)
    } catch (error) {
      console.error(error)
    }

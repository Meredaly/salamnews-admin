<script setup lang="ts">
  ...
  const selectCategoryAction = (action: ActionMenu, category: Category, parentId: string | null) => {
  ...
  const onDropSubCategory = (index: number, dropResult: CategoryDropResult) => {
    categories.value[index].sub_categories = applyDrag(categories.value[index].sub_categories, dropResult)

    updateCategoriesPosition(categories.value[index].sub_categories)
  }
  ...
  const updateCategoriesPosition = async (categories: Category[]) => {
    try {
      ...
      await UPDATE_CATEGORIES_POSITION({ data: categoriesNewPosition })
    } catch (error) {
      console.error(error)
    }
  }
          ...
          >
            ...
            >
              <div class="categories__list-button">
                ...
                <div class="categories__list-action">
                  <dropdown-dots
                    @select-item="(action) => selectCategoryAction(action, category, null)"
                    :menu="actions"
                  />
                </div>
              </div>
              <Container
                @drop="(dropResult:CategoryDropResult) => onDropSubCategory(index, dropResult)"
                lock-axis="y"
                drag-handle-selector=".categories__list-drag"
                v-if="selectedIndex === index"
                class="categories__list-menu"
              >
                <Draggable
                  v-for="(subCategory, indexSub) in category.sub_categories"
                  class="categories__list-item categories__list-item_sub"
                >
                  <div class="categories__list-button">
                    <div class="categories__list-drag">
                      <icon name="unfoldMore" />
                    </div>
                    <div class="categories__list-number">{{ indexSub + 1 }}</div>
                    <div class="categories__list-menu-action"></div>
                    <div class="categories__list-name">{{ getCategoryNameByLang(subCategory) }}</div>
                    <div class="categories__list-action">
                      <dropdown-dots
                        @select-item="(action) => selectCategoryAction(action, subCategory, category.category_slug)"
                        :menu="actions"
                      />
                    </div>
                  </div>

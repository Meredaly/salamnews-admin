<script setup lang="ts">
  ...
  import ArticlePin from '@/components/article/ArticlePin.vue'
  ...
  import { IMAGE_UPLOAD } from '@/api/imageUploader'
      ...
      article.value.category_id =
        data.category?.map((category: any) => {
          ...
          selectedCategories.value.push({
            ...
            name: parentName + `${locale.value === 'tm' ? category.name_tm : category.name_ru}`
          })
  ...
  const updateArticle = async () => {
    try {
      ...
      await UPDATE_ARTICLE({
        data: {
          ...article.value,
          category_id: article.value.category_id.length ? article.value.category_id : []
        }
      })
  ...
  const pinArticle = (position: number) => {
    article.value.pin = position
  }
  ...
  const updateContent = async (): Promise<any> => {
    try {
      ...
      if (data.content_dto) {
        ...
        let newContent = data.content_dto
          .sort((a: any, b: any) => a.order_number - b.order_number)
          .map((contentItem: any) => {
            contentItem.method = 'UPDATE'
            return contentItem
        ...
        newContent = newContent.map((newContentElement: any, index: number) => {
          if (newContentElement.type === content.value[index].type) {
            newContentElement = {
              ...content.value[index],
              uuid: newContentElement.uuid,
              method: 'UPDATE'
            }
          }
        ...
        const isSimilar = newContent.every((newContentElement: any, index: number) => {
          return newContentElement.uuid === content.value[index].uuid
        })

        if (newContent.length === content.value.length && isSimilar) {
          if (JSON.stringify(newContent) !== JSON.stringify(content.value)) {
            content.value = newContent
          }
        } else {
  ...
  const changeContentVideo = async (index: number, file: File) => {
    ...
    if (data.status === 'SUCCESS') {
      ...
      content.value[index].duration = data.duration
          ...
          <div class="article__info-pin">
            ...
            <div class="article__pin">
              <s-button
                ...
                :title="t('unPin')"
              />
              <article-pin @pin="pinArticle" v-else />
            </div>
          </div>
